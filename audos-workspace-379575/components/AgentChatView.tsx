...
export default function AgentChatView({ runtime }: AgentChatViewProps) {
  ...
  const composerPlaceholder = pendingAttachments.length > 0
    ...
    : isBeaconSpace
      ...
      : 'Ask Spark to generate, refine, save, or load a scene…';
          ...
          <div className="mx-auto w-full max-w-2xl px-6 sm:px-10 py-10" data-testid="home-view">
            <div className="text-center mb-8">
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--space-surface-accent-soft)] shadow-sm">
                <Sparkles className="h-6 w-6 text-[var(--space-text-brand)]" />
              </div>
              <h1 className="text-2xl font-semibold text-[var(--space-text-primary)]">What should we animate?</h1>
              {homeWelcomeText && (
          ...
          <div className="mt-8 px-4">
            {isBeaconSpace ? (
              ...
              <div className="text-center text-[var(--space-text-secondary)]">
                <Sparkles className="w-12 h-12 mx-auto mb-3 text-[var(--space-text-muted)]" />
                <p className="text-sm">Give Spark a motion idea—or ask how NeoFlash works.</p>
                {starterPrompts.length > 0 && (
                  <div className="mt-5 flex flex-wrap justify-center gap-2">
                    {starterPrompts.map((prompt) => (
                      <button
